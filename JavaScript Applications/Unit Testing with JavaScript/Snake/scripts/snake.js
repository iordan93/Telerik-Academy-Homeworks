...
var snakeGame = (function () {
    ...
    var Snake = MovingGameObject.extend({
        ...
        checkSnakeCollision: function () {
            for (i = 1; i < this.size; i++) {
                ...
                var collidesWithCurrentPiece = this.head.position.x == currentPiece.position.x && this.head.position.y == currentPiece.position.y;
    ...
    var SnakeGame = Class.create({
        ...
        initGameObjects: function () {
            ...
            // To fix the collision detection bug, the snake should have a die handler each time the game starts.
            // If this function is in the attachHandlers() method, it will not be attached upon restart
            ...
            for (var i = 0; i < this.gameObjects.length; i += 1) {
                ...
                if (!(gameObject instanceof Snake)) {
                    ...
                    if (colliding && (gameObject instanceof Food)) {
                        this.generateRandomFood(gameObject);
                    }
                }
            }
        },

        checkBordersCollision: function () {
            var i;
            for (i = 0; i < this.snake.size; i += 1) {
                var piece = this.snake.pieces[i];
                var outOfXRange = piece.position.x < 0 || piece.position.x + piece.size > this.maxX;
                var outOfYRange = piece.position.y < 0 || piece.position.y + piece.size > this.maxY;

                // Update only the needed coordinates to avoid unnecessary checks
                if (outOfXRange) {
        ...
        generateRandomFood: function (gameObject) {
            var position = {
                x: (Math.random() * this.maxX) | 0,
                y: (Math.random() * this.maxY) | 0
            };
        ...
        start: function () {
            ...
            this.snakeTimer = setInterval(function () {
                ...
                self.snake.checkSnakeCollision();
                self.redraw();
            }, 50);
